...
RUN curl -O -J -L  http://download.jboss.org/wildfly/10.0.0.Final/wildfly-10.0.0.Final.zip && unzip wildfly-10.0.0.Final.zip -d /usr/local
...
COPY  target/jaxrs-1.0-SNAPSHOT.war  /usr/local/jaxrs-1.0-SNAPSHOT.war
COPY  src/main/docker/batchcommand.txt  /usr/local/batchcommand.txt
